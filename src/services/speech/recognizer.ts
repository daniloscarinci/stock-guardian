 ...
 * `voiceAllowOnline` setting. It is false unless a person has switched it on -
 * in Settings, or on the panel that appears when a listen failed for want of a
 * local model, which is where somebody actually is when the question arises.
 * Either way the label names Google rather than saying "online", and nothing in
 * this application ever writes that setting on its own: a failure may offer the
 * switch, and only a person flips it. While it is false nothing changes at all:
 * the Android plugin sends EXTRA_PREFER_OFFLINE and webspeech.ts sets
 * `processLocally = true`, both exactly as they did when there was no other
 * mode. The default path is not weakened by the existence of the other one - it
 * is the same code, with the same tests on it.
 */
